...
public class RightArmPoseGUI
{
    public static final int SIZE = 54;
    public static final String TITLE = "Right Arm Pose Editor";
    ...
    {
        return new  EulerAngle(Math.toDegrees(-15), Math.toDegrees(0), Math.toDegrees(10));
    }

    public static void refresh(Inventory inventory, EditorSession session)
    {
        //弧度(Radians)
        EulerAngle pose = session.getArmorStand().getRightArmPose();

        //轉成角度
        inventory.setItem(INFO, new ItemBuilder(Material.STICK)
                .setName("Right Arm Pose")
                .setLore("",
