...
public class RightLegPoseGUI
{
    public static final int SIZE = 54;
    public static final String TITLE = "Right Leg Pose Editor";
    ...
    {
        return new  EulerAngle(Math.toDegrees(1), Math.toDegrees(0), Math.toDegrees(1));
    }

    public static void refresh(Inventory inventory, EditorSession session)
    {
        //弧度(Radians)
        EulerAngle pose = session.getArmorStand().getRightLegPose();

        //轉成角度
        inventory.setItem(INFO, new ItemBuilder(Material.STICK)
                .setName("Right Leg Pose")
                .setLore("",
